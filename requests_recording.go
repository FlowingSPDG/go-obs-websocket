...
type GetRecordingStatusRequest struct {
	...
	response chan GetRecordingStatusResponse
}
...
func NewGetRecordingStatusRequest() GetRecordingStatusRequest {
	return GetRecordingStatusRequest{
		_request{
			ID_:   GetMessageID(),
			Type_: "GetRecordingStatus",
			err:   make(chan error, 1),
		},
		make(chan GetRecordingStatusResponse, 1),
	}
}
...
func (r *GetRecordingStatusRequest) Send(c Client) error {
	...
	go func() {
		m := <-future
		var resp GetRecordingStatusResponse
...
func (r GetRecordingStatusRequest) Receive() (GetRecordingStatusResponse, error) {
	if !r.sent {
		return GetRecordingStatusResponse{}, ErrNotSent
	}
	if receiveTimeout == 0 {
		select {
		case resp := <-r.response:
			return resp, nil
		case err := <-r.err:
			return GetRecordingStatusResponse{}, err
		}
	} else {
		select {
		case resp := <-r.response:
		...
		case <-time.After(receiveTimeout):
			return GetRecordingStatusResponse{}, ErrReceiveTimeout
		}
	}
}
