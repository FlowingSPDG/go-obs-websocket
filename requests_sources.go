...
func NewSetAudioTracksRequest(
	...
	return SetAudioTracksRequest{
		sourceName,
		track,
		active,
		_request{
			ID_:   GetMessageID(),
			Type_: "SetAudioTracks",
			err:   make(chan error, 1),
		},
		make(chan SetAudioTracksResponse, 1),
	}
}
...
func (r *SetAudioTracksRequest) Send(c Client) error {
	...
	go func() {
		m := <-future
		var resp SetAudioTracksResponse
		if err = mapToStruct(m, &resp); err != nil {
			r.err <- err
		} else if resp.Status() != StatusOK {
			r.err <- errors.New(resp.Error())
...
func (r SetAudioTracksRequest) Receive() (SetAudioTracksResponse, error) {
	if !r.sent {
		return SetAudioTracksResponse{}, ErrNotSent
	}
	if receiveTimeout == 0 {
		select {
		case resp := <-r.response:
			return resp, nil
		case err := <-r.err:
			return SetAudioTracksResponse{}, err
		}
	} else {
		select {
		case resp := <-r.response:
		...
		case err := <-r.err:
			return SetAudioTracksResponse{}, err
		case <-time.After(receiveTimeout):
			return SetAudioTracksResponse{}, ErrReceiveTimeout
		}
	}
}
...
func NewGetAudioTracksRequest(sourceName string) GetAudioTracksRequest {
	return GetAudioTracksRequest{
		sourceName,
		_request{
			ID_:   GetMessageID(),
			Type_: "GetAudioTracks",
			err:   make(chan error, 1),
		},
		make(chan GetAudioTracksResponse, 1),
	}
}
...
func (r *GetAudioTracksRequest) Send(c Client) error {
	...
	go func() {
		m := <-future
		var resp GetAudioTracksResponse
		if err = mapToStruct(m, &resp); err != nil {
			r.err <- err
		} else if resp.Status() != StatusOK {
			r.err <- errors.New(resp.Error())
...
func (r GetAudioTracksRequest) Receive() (GetAudioTracksResponse, error) {
	if !r.sent {
		return GetAudioTracksResponse{}, ErrNotSent
	}
	if receiveTimeout == 0 {
		select {
		case resp := <-r.response:
			return resp, nil
		case err := <-r.err:
			return GetAudioTracksResponse{}, err
		}
	} else {
		select {
		case resp := <-r.response:
		...
		case err := <-r.err:
			return GetAudioTracksResponse{}, err
		case <-time.After(receiveTimeout):
			return GetAudioTracksResponse{}, ErrReceiveTimeout
		}
	}
}
...
func NewGetSourceActiveRequest(sourceName string) GetSourceActiveRequest {
	return GetSourceActiveRequest{
		sourceName,
		_request{
			ID_:   GetMessageID(),
			Type_: "GetSourceActive",
			err:   make(chan error, 1),
		},
		make(chan GetSourceActiveResponse, 1),
	}
}
...
func (r *GetSourceActiveRequest) Send(c Client) error {
	...
	go func() {
		m := <-future
		var resp GetSourceActiveResponse
		if err = mapToStruct(m, &resp); err != nil {
			r.err <- err
		} else if resp.Status() != StatusOK {
			r.err <- errors.New(resp.Error())
...
func (r GetSourceActiveRequest) Receive() (GetSourceActiveResponse, error) {
	if !r.sent {
		return GetSourceActiveResponse{}, ErrNotSent
	}
	if receiveTimeout == 0 {
		select {
		case resp := <-r.response:
			return resp, nil
		case err := <-r.err:
			return GetSourceActiveResponse{}, err
		}
	} else {
		select {
		case resp := <-r.response:
		...
		case err := <-r.err:
			return GetSourceActiveResponse{}, err
		case <-time.After(receiveTimeout):
			return GetSourceActiveResponse{}, ErrReceiveTimeout
		}
	}
}
