...
// SleepRequest : Waits for the specified duration
// Designed to be used in `ExecuteBatch` operations.
//
...
func (r SleepRequest) Receive() (SleepResponse, error) {
	if !r.sent {
		return SleepResponse{}, ErrNotSent
	}
	if receiveTimeout == 0 {
		select {
		case resp := <-r.response:
		...
		case err := <-r.err:
			return SleepResponse{}, err
		case <-time.After(receiveTimeout):
			return SleepResponse{}, ErrReceiveTimeout
		}
	}
}
